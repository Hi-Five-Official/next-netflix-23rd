...
interface TrendingCarouselProps {
  items: TmdbMedia[];
  onCurrentChange: (id: number, mediaType: "movie" | "tv") => void;
}

const TrendingCarousel = ({ items, onCurrentChange }: TrendingCarouselProps) => {
  const [current, setCurrent] = useState(0);

  useEffect(() => {
    const item = items[current];
    onCurrentChange(item.id, item.media_type);
  }, [current, items, onCurrentChange]);
